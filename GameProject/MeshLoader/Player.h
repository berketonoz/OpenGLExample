...
//            cout << objects[i].IsWall() << endl;
                ...
                if (this->box.overlaps(objects[i].GetAABB())){ //If collision is detected between any object and the player
                    if (!objects[i].IsWall()){ //If the object is not a wall
                        if(!objects[i].GetCollected()){ //If we have not yet collected it
                            objects[i].SetVisible(false);
                            score += 100;
                            objects[i].SetCollected(true);
                        }
                    }
                    else{ //If the object we hit is a wall
                        this->bump = true;
                        if(!objects[i].GetHit()){
                            this->life -= 1;
...
//                        }
    ...
    void Draw(Shader shader, glm::vec3 position, glm::vec3 camera_position) {
        if(this->visible == true){ //If the object is on sight
            std::vector<float> mins = this->GetModel().GetMins();
            std::vector<float> maxs = this->GetModel().GetMaxs();
            glm::vec3 pos = this->GetPosition();
    ...
    void RunPhysics(Object surface, float deltatime){
        cout << this->moving_left << " " << this->moving_right << endl;
